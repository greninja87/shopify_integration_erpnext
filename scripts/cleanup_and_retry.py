"""
DEV UTILITY — bench console helper to wipe all transactional data and re-run
Shopify Logs from scratch during development.

Usage (bench console only):
    from shopify_integration.scripts.cleanup_and_retry import cleanup_all
    cleanup_all()

WARNING: This deletes EVERY Payment Entry, Sales Invoice, Delivery Note,
Sales Order, Customer, Address, and Contact in the ERPNext instance.
It is completely irreversible and will destroy production data if run there.
"""
...
def cleanup_all():
    if not frappe.conf.get("developer_mode"):
        raise RuntimeError(
            "cleanup_and_retry.py must only be run in developer mode. "
            "Set developer_mode = 1 in site_config.json to enable it."
        )
